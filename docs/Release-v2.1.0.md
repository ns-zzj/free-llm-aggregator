...
- **上游 `baseUrl` 不再自动补 `/v1`**：四种上游协议统一成「**baseUrl 当完整前缀，我们只在后面拼端点**」
  ...
  ⚠ **从 2.0.0 升级的人要检查这两种上游的地址**：以前填 `https://api.anthropic.com` 也能跑，现在必须自己写成
  `https://api.anthropic.com/v1`（`openai-responses` 同理）。改完看提供商弹层里那行「实际请求：…」核对一下就知道对不对。
  数据库不用动、数据目录不用动（迁移在启动时自动跑）。
- 后台「提供商」弹层里，**`baseUrl` 下面多一行小字「实际请求：…」**：按当前 baseUrl + 选的适配器
  把最终打出去的地址拼出来（Cloudflare 那种模型名在路径里的也会显示），填错一眼能看见。
- **容器基底从 Debian 换成 Alpine，镜像小了约 18 MB**（包 102 MB → 84 MB）。对使用者**没有任何影响**：
  同一条 `docker load`、同一条 `docker run`（参数一字不改）、同一个数据目录、同一个 `nszzj-free-llm-aggregator` 标签。
...
```bash
# ① 导入镜像（<架构> 二选一：x86-64 或 arm64，照附件写）
...
- **升级过来的**：数据库迁移在启动时自动跑，不用手动执行任何东西；要动的地方只有上游地址那一条
  （见「更新日志」里的 ⚠）。后台那个新的「探测 / 测试的超时」不用管，默认值就是照真实请求设的。

## 已知问题

- **镜像只提供 x86-64 和 arm64 两种**（就是附件里那两个）：32 位（x86-32 / arm32）没有 ——
  Node 官方从 Node 16 之后基本不再发 32 位 Linux 的构建，`node:22` 根本没有 386 那份，
  我们自己也没法凭空变一个出来。要 32 位请自己编译，见 README「跑起来 → B. 开发者」。
