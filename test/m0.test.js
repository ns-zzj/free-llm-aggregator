...
/**
 * M0 端到端测试：假上游 + 真服务
 * 覆盖：口令鉴权、apiKey 加密入库、/v1/models、非流式对话、失败自动换源、来源冷却、假数据端点
 */
...
test('1) 未带访问口令访问 /v1/models → 401', async () => {
...
test('2) 错误口令 → 401', async () => {
  const res = await fetch(`${baseUrl}/v1/models`, { headers: { authorization: 'Bearer wrong-key' } });
  assert.equal(res.status, 401);
});
...
test('4b) 「密码」页用的一条口令：能取回明文/掩码，更换后旧口令立即失效', async () => {
  ...
  const old = await fetch(`${baseUrl}/v1/models`, { headers: { authorization: `Bearer ${ACCESS_KEY}` } });
  assert.equal(old.status, 401, '旧口令应立即失效');
  const fresh = await fetch(`${baseUrl}/v1/models`, { headers: { authorization: 'Bearer sk-my-new-key-123456' } });
  ...
  const legacyAuth = await fetch(`${baseUrl}/v1/models`, { headers: { authorization: 'Bearer sk-my-new-key-123456' } });
  ...
  const restored = await fetch(`${baseUrl}/v1/models`, { headers: { authorization: `Bearer ${ACCESS_KEY}` } });
  assert.equal(restored.status, 200);
});

test('5) GET /v1/models 输出去重后的模型（All + 带类别前缀的模型名）', async () => {
...
test('11) 未知模型 → 404；名字写不全 → 404；假数据端点正常；未实现端点 404', async () => {
  ...
  const usage = await fetch(`${baseUrl}/v1/usage`, { headers: { authorization: `Bearer ${ACCESS_KEY}` } });
  assert.equal(usage.status, 200);
  assert.equal((await usage.json()).object, 'list');

  const notImplemented = await fetch(`${baseUrl}/v1/embeddings`, {
    method: 'POST',
